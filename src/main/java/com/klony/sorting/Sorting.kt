...
data class Sorting(val mode: Mode, val direction: Direction) {
    companion object {
        ...
        fun help(separator: String = ", ") = enumValues<Mode>().joinToString(separator = separator) { mode ->
    ...
    fun sortFiles(files: Set<File>) = when (this.mode) {
    ...
    private inline fun <T, R : Comparable<R>> Iterable<T>.sortedWithDirectionBy(direction: Direction, crossinline selector: (T) -> R?) = when (direction) {
